...
zread_ipv4_add (struct zserv *client, u_short length)
{
 ...
 /* @PEMP  */
  u_char weight;
  u_char default_route_flag;
  u_char clubmed_community=0;
      ...
      for (i = 0; i < nexthop_num; i++)
	{
	  ...
	  switch (nexthop_type)
	    ...
	    case ZEBRA_NEXTHOP_IPV4:
	      ...
	      break;
		/*@PEMP*/
    	case ZEBRA_NEXTHOP_IPV4_WEIGHT:
    		weight = 0;
    		default_route_flag = 0;
    		clubmed_community = 1;
    		nexthop.s_addr = stream_get_ipv4 (s);
    		weight = stream_getc (s);
    		default_route_flag = stream_getc (s);
    		zlog_debug ("weight = %d default flag = %d",weight,default_route_flag);
    		weight_nexthop_ipv4_add (rib, &nexthop, weight,default_route_flag,NULL);
    		break;
		/*end PEMP*/		  
	    case ZEBRA_NEXTHOP_IPV4_IFINDEX:
	      nexthop.s_addr = stream_get_ipv4 (s);
  ...
  if (clubmed_community)
	  rib->clubmed_community_id =  stream_getc (s);
