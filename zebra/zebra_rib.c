...
/*@PEMP*/
struct nexthop *
weight_nexthop_ipv4_add (struct rib *rib, struct in_addr *ipv4, u_char w, u_char f, struct in_addr *src)
{
  struct nexthop *nexthop;
  ...
  nexthop->gate.ipv4 = *ipv4;
  nexthop->weight = w;
  nexthop->default_route_flag = f;
  zlog_debug ("nexthop->weight = %d default route %d",nexthop->weight,nexthop->default_route_flag);
  if (src)
    nexthop->src.ipv4 = *src;
  ...
  if (select && select == fib)
    {
	 ...
	 /*@PEMP */
	  zlog_debug ("rib_process: Updating existing route");
	  /*end PEMP */
