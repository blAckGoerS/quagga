...
/* @PEMP defined constant */

#define PEMP_SUPPORT 			1
#define MAX_PEERING_LINK 		3
#define MAX_ROUTEGAME 			2
...
struct pemp_community
{
	...
	struct prefix 	network_prefix[MAX_PREFIX];
};

struct pemp_routing_game
{
  	u_int8_t 		game_id;
  	u_int8_t 		policy;
  	u_int8_t 		uLB;
  	float 			threshold;
	u_int8_t  		number_of_path;
	u_int8_t  		community_id;
	u_int8_t  		peer_community_id;
	u_int8_t  		status;
};

struct pemp_route_cost
{
	u_int32_t 		incost;		// cost from border router to core router
	u_int32_t 		egcost;		// cost from core router to border router
	u_int32_t 		Pincost;	// peer's ingress cost
	u_int32_t 		Pegcost;	// peer's egress cost
	u_int8_t  		active;		// status of cost entry
	struct in_addr 	if_ipa;  	// ip address of core router's NIC that connect to the border router
	struct in_addr 	brouter_id; // ip address of the border router
	u_int8_t 		game_id;
};

struct pemp_igp_cost {
	u_int32_t 		incost;		// cost from border router to core router
	u_int32_t 		egcost;		// cost from core router to border router
	u_int8_t  		active ;	// status of this route cost entry
	struct in_addr 	brouter_id; // ip address of border router
};
...
#define BGP_CONFIG_DAMPENING              (1 << 0)
  ...
  /* @PEMP data structure to support routing game */
  /* mostly to support configure write command in bgpd.c */
  u_int8_t 							bgp_pemp_enable;
  u_int8_t							number_of_community;
  u_int8_t  						number_of_peer_community;
  u_int8_t  						number_of_game;
  struct pemp_routing_game 			*routing_game_info;
  struct pemp_community 			*local_community;
  struct pemp_community 			*peer_community;
