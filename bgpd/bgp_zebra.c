...
/* @PEMP Growable buffer for nexthops' weight sent to zebra */
struct stream *bgp_nhweight_buf = NULL;
...
bgp_zebra_announce (struct prefix *p, struct bgp_info *info, struct bgp *bgp, safi_t safi)
{
  ...
  /* resize nexthop weight buffer size if necessary */
	...
	{
       w_newsize = (sizeof (u_char *) * (bgp_info_mpath_count (info) + 1));
	...
	stream_reset ( bgp_nhweight_buf);

	/* resize nexthop community id buffer size if necessary */
	if ((c_oldsize = stream_get_size ( bgp_nhflag_buf)) <
			(sizeof (u_char *) * (bgp_info_mpath_count (info) + 1)))
	{
		c_newsize = (sizeof (u_char *) * (bgp_info_mpath_count (info) + 1));
  ...
  if (p->family == AF_INET)
    {
      ...
      if (wflag)
      {
    	  SET_FLAG (api.message, ZAPI_MESSAGE_WEIGHT);
    	  api.weight = (struct u_char **)STREAM_DATA (bgp_nhweight_buf);
    	  api.default_route_flag = (struct u_char **)STREAM_DATA (bgp_nhflag_buf);
      }
      /* end PEMP*/
      ...
      /* @PEMP */
      if (wflag)
    	  api.pemp_community_id = info->attr->pemp_community_id;
      /* end PEMP */
...
bgp_zebra_init (void)
{
  ...
  bgp_nhweight_buf 	= stream_new(BGP_NHWEIGHT_BUF_SIZE);
  bgp_nhflag_buf 	= stream_new(BGP_NHFLAG_BUF_SIZE);
}
