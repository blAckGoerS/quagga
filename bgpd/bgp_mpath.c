...
/* @PEMP */
void
bgp_info_mpath_update_rg (struct bgp_node *rn, struct bgp_info *new_best,
                       ...
                       struct bgp_maxpaths_cfg *mpath_cfg)
{
	  ...
	  //clear the current multipath list - that lead by old_best
	  while (cur_mpath)
	  {
		  if (!cur_mpath)
			  break;
	  ...
	  // scan through all the nodes in mp_list (generated by best_selection)
	  // add nodes to the multi-path list that lead by new_best
	  // REMEMBER: only add node that different than the new_select
	  int count = 0;
	  while (mp_node)
	  {
