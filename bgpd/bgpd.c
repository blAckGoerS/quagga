...
bgp_config_write (struct vty *vty)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
      ...
      for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
	{
	  ...
	  /* @PEMP write to configuration file */
      if ( bgp->bgp_pemp_enable)
      {
    	  ...
    	  {
    		  int i,j,k;
    		  struct pemp_routing_game 	*game;
    		  struct pemp_community 	*community;
    		  struct pemp_community 	*peer_community;

    		  game = bgp->routing_game_info;
    		  for (i=0;i<bgp->number_of_game;i++)
    		  {
    			  community = bgp->local_community;

    			  // add community
    			  for (j=0;j<bgp->number_of_community;j++)
    			  {
    				  if ( game->community_id == community->id )
    				  {
    					  ...
    					  // add selected customer network to a local community
    					  for (k=0;k < community->number_of_prefix;k++)
    					  {
    						  struct prefix *clubmednet;
    					      char buf[SU_ADDRSTRLEN];
    					      clubmednet = &(community->network_prefix[k]);
    					      vty_out (vty, " local network %s/%d community %d%s",
    					      	inet_ntop (clubmednet->family, &clubmednet->u.prefix, buf, SU_ADDRSTRLEN),
    					      	clubmednet->prefixlen,community->id,VTY_NEWLINE);
    					  }
    					  break;
    				  }
    				  community++;
    			  ...
    			  peer_community = bgp->peer_community;
    			  // associating peer customer network with peer's community
    			  for (j=0;j<bgp->number_of_peer_community;j++)
    			  {
    				  if ( game->peer_community_id == peer_community->id )
    				  {
    					  // add peer clubmed network
    					  for (k=0;k < peer_community->number_of_prefix;k++)
    					  {
    						  struct prefix *pclubmednet;
    					      char buf[SU_ADDRSTRLEN];
    					      pclubmednet = &(peer_community->network_prefix[k]);
    					      vty_out (vty, " peer network %s/%d community %d%s",
    					      	inet_ntop (pclubmednet->family, &pclubmednet->u.prefix, buf, SU_ADDRSTRLEN),
    					      	pclubmednet->prefixlen,peer_community->id,VTY_NEWLINE);
    					  }
    					  break;
    				  }
    				  peer_community++;
