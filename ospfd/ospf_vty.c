...
DEFUN (pemp_border_router_id,
       ...
       "Specify PEMP border router id \n"
       "Border router-id in IP address format\n")
{
  struct ospf *ospf = vty->index;
  struct in_addr router_id;
  int ret;
  ...
  if (!ret)
    {
      vty_out (vty, "Please specify Border Router ID by A.B.C.D%s", VTY_NEWLINE);
      return CMD_WARNING;
    }
	
  ospf->pemp_support = 1 ;
  add_bRouter_inaddr(router_id); 
  ...
  if (ospf != NULL)
    {
	  ...
	  /* @PEMP -- not tested yet */
	  if ( ospf->pemp_support == 1 )
	  {
		int i;
		for ( i = 0; i < get_bRouter_index(); i++)
		{
			vty_out (vty, "border router-id %s", 
                 inet_ntoa(get_bRouter_ip(i)) , VTY_NEWLINE);
		}
	  }
	  /* end PEMP */
...
ospf_vty_init (void)
{
  ...
  /* @PEMP support commands */
  install_element (OSPF_NODE, &pemp_border_router_id_cmd);
