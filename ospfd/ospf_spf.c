...
{
	int i; 
	for ( i = 0; i < MAX_NPROFILE; i++)
	{
		b_router_list[i].status = 0;
	}
}

int get_bRouter_index()
{
	return b_router_index;
}

/*when calling this function make sure that i must < b_router_index 
  called by ospf_config_write() in the ospf_vty.c to write to configuration file */
struct in_addr get_bRouter_ip(int i)
{
	return b_router_list[i].b_router_id;
}

int add_bRouter_inaddr(struct in_addr b_router_id)
{
	int i = b_router_index;
	if ( i > MAX_NPROFILE )
		return -1;
	if ( lookup_bRouter(b_router_id) != -1 ) //border router have been added in the list 
		return -1;
		
	b_router_list[i].b_router_id = b_router_id;
	b_router_list[i].i_rcost = 0;
	b_router_list[i].status = 0;
	b_router_index++;
	return i; 	//return the position that have been updated 
} 
int add_bRouter(const char *ipa) 
{
	int i = b_router_index;
	...
	int ret = inet_aton (ipa,&b_router_id);
...
int lookup_bRouter(struct in_addr router_id) 
{
	int i; 
	for ( i = 0; i < MAX_NPROFILE; i++)
	{
		if ( b_router_list[i].b_router_id.s_addr == router_id.s_addr )
			return i;
	}
	return -1; // not found 
}

int update_b_router_cost(int cost, struct in_addr router_id)
{
	int i = lookup_bRouter(router_id);
	if ( i != -1 )
	{
		b_router_list[i].i_rcost = cost;
		b_router_list[i].status = 1;
	}
	return i;
}

int get_ingress_cost(struct in_addr b_router_id)
{
	int i = lookup_bRouter(b_router_id);
	if ( i == -1 )
		return 0;
	else 
...
/* @PEMP modified ospf_spf_next() function to support ingress routing cost calculation*/
static void
ospf_spf_next2 (struct vertex *v, struct ospf_area *area,
	       struct pqueue *candidate)
{
  struct ospf_lsa *w_lsa = NULL;
  u_char *p;
  ...
  if (v->type == OSPF_VERTEX_ROUTER)
    ...
    zlog_debug ("%s: Next vertex of %s vertex %s",
                __func__, v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
                inet_ntoa(v->lsa->id));
  ...
  while (p < lim)
    {
      ...
      if (IS_LSA_MAXAGE (w_lsa))
		...
		/* (c) If vertex W is already on the shortest-path tree, examine
         the next link in the LSA. */
		 /* need to refer back to the lsdb and check for the stat of ospf_lsa 
		...
		*(v->stat) = LSA_SPF_IN_SPFTREE;
		...
		for (;;)
	    {
			...
			*(v->stat) = LSA_SPF_IN_SPFTREE;
			...
			{
			    zlog_debug (" Found distance to real Root [clubmed router]- Terminate ");
