                 ...
                 struct zapi_ipv4 *api)
{
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_NEXTHOP))
    {
	  ...
	  /*@PEMP*/
      if (CHECK_FLAG (api->message, ZAPI_MESSAGE_WEIGHT) )
      {
		  zlog_debug ("ZAPI_MESSAGE_WEIGHT");
    	  for (i = 0; i < api->nexthop_num; i++)
    	  {
    		  stream_putc (s, ZEBRA_NEXTHOP_IPV4_WEIGHT); 
    		  // a special flag for nexthop that have associated weight
    		  // follow each next hop is its weight 
    		  stream_put_in_addr (s, api->nexthop[i]);
    		  stream_putc (s, api->weight[i]);
    		  stream_putc (s, api->default_route_flag[i]);
    	  }
      } /* end PEMP*/
      else
      { 
  ...
  /* @PEMP */
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_WEIGHT))
	  stream_putc (s, api->pemp_community_id);
  /*end PEMP*/
